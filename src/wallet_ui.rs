...
<section class="vue" id="vue-recevoir" hidden>
  ...
  <div class="note">
    ...
    <p>
      Le minage en dérive aussi une par bloc trouvé, pour la même raison. Ces
      adresses-là ont déjà reçu leur récompense et n'ont pas besoin d'être
      distribuées&nbsp;: elles sont rangées ci-dessous, et seules celles que
      vous avez demandées ou nommées sont montrées.
    </p>
  </div>
  <div class="chercheur">
    ...
    <div class="boutons" id="zone-minage" hidden>
      <button type="button" class="plat" id="bouton-minage">Afficher aussi les adresses de minage</button>
    </div>
  </div>
...
<section class="vue" id="vue-reseau" hidden>
  ...
  <div class="carte">
    <div class="mineur">
      <div class="titre">
        <h3 id="joignable-titre">Votre portefeuille aide le réseau</h3>
        <p id="joignable-sous">Il accepte les connexions des autres, pour que le réseau ne dépende pas d'une seule machine.</p>
      </div>
      <button type="button" class="bascule" id="bascule-joignable" aria-pressed="false"
              aria-label="Rendre ce nœud joignable"><i></i></button>
    </div>
    <div class="n" id="joignable-etat" style="margin-top:.4rem"></div>
    <div class="note">
      <h3>Ce que fait ce réglage</h3>
      <p>
        Quand il est allumé, votre portefeuille demande à votre box d'ouvrir son
        accès, et devient un point d'entrée du réseau&nbsp;: les autres peuvent
        se connecter à vous. C'est ce qui permet au réseau de vivre sans dépendre
        d'un serveur unique. La plupart des box ouvrent l'accès toutes seules.
      </p>
      <p>
        En échange, votre <strong>adresse internet devient visible</strong> des
        autres nœuds — c'est le fonctionnement normal d'un réseau pair-à-pair. Si
        vous préférez rester un simple client, éteignez&nbsp;: votre portefeuille
        marche exactement pareil, il profite des nœuds ouverts par les autres.
      </p>
      <p>Un changement <strong>prend effet au prochain lancement</strong> du portefeuille.</p>
    </div>
  </div>
  <div class="heros">
  ...
  <div class="note">
    ...
    <p>
      ...
      pour qu'un bloc tombe toutes les deux minutes. Comparé à ce que fait
      <em>votre</em> machine, il donne la part qu'elle représente, indiquée ci-dessus.
    </p>
  </div>
</section>
...
const I18N_MOTIFS = {
  "en": [
    ...
    [/^soit environ (.+) % de la puissance mesurée du réseau$/, "about $1 % of the network's measured power"],
  ...
  "ja": [
    ...
    [/^(.+) adresse\(s\) demandée\(s\) · (.+) de minage rangée\(s\)$/, "要求したアドレス $1 件 · マイニング用 $2 件は畳まれています"],
    [/^(.+) adresse\(s\) demandée\(s\)$/, "要求したアドレス $1 件"],
    [/^(.+) adresse\(s\) au total, dont (.+) de minage$/, "合計 $1 件のアドレス (うちマイニング用 $2 件)"],
    [/^Afficher aussi les (.+) adresse\(s\) de minage$/, "マイニング用アドレス $1 件も表示する"],
    [/^Aucune adresse demandée pour l'instant\. Le bouton « Nouvelle adresse » en crée une, que vous pourrez nommer\. Vos (.+) adresse\(s\) de minage sont rangées : elles ont reçu vos récompenses et comptent dans votre solde\.$/, "要求したアドレスはまだありません。「新しいアドレス」ボタンで作成し、名前を付けられます。マイニング用アドレス $1 件は畳まれています。報酬を受け取っており、残高に含まれます。"],
    ...
    [/^soit environ (.+) % de la puissance mesurée du réseau$/, "ネットワークの測定パワーの約 $1 %"],
...
async function rafraichir(){
  ...
  try{
    ...
    if (sync.synchronise){
      ...
      // --- Seul pour deux raisons opposees, qui s'affichaient pareil.
      //
      // Un nœud sans pair n'a pas l'adresse de personne, ou bien il a frappe
      // et personne n'a ouvert. Le premier cas se repare en deux lignes ; le
      // second demande de regarder le serveur ou le reseau. Tant que les deux
      // disaient « aucun ordinateur joignable », le nouveau venu concluait que
      // le reseau etait mort — alors qu'il lui manquait un fichier. C'est
      // l'abandon le plus probable d'un premier lancement, et le seul que la
      // page pouvait eviter toute seule.
      const aveugle = seul && Number(info.amorces_configurees) === 0;
      ...
      document.getElementById("recup-titre").textContent = aveugle
      ...
      // s'adresse a lui par une condition qu'il reconnaitra. Sauf si le nœud
      // est aveugle : lui promettre que ses fonds reviendront « a la fin de
      // cette etape » serait une fausse assurance, l'etape ne pouvant pas
      // commencer.
      document.getElementById("recup-restaure").hidden =
        aveugle || Number(solde.adresses_derivees) > 2;
      document.getElementById("sync-detail").textContent = aveugle
        ? "Le programme ne contient l'adresse d'aucun serveur, et c'est voulu : " +
          "une adresse gravée dans un logiciel distribué deviendrait une dépendance " +
          "permanente envers celui qui la tient. Elle se donne à côté, dans un fichier " +
          "que vous pouvez modifier. Créez q21-data/amorces.txt auprès du programme, " +
          "écrivez-y la ligne amorce.q21.dev:21121, puis relancez. REJOINDRE.md, livré " +
          "avec le programme, détaille ce geste et la façon d'ajouter d'autres points d'entrée."
        : seul
...
document.getElementById("bouton-adresse").addEventListener("click", async () => {
  ...
  try{
    ...
    // La nouvelle adresse rejoint le carnet tout de suite, et son champ de
    // nom s'ouvre : c'est le moment ou l'on sait encore pour qui elle est.
    try{
      carnetFiltre = "";
      document.getElementById("filtre-adresses").value = "";
      await listerAdresses();
      const neuve = carnet.find(e => e.adresse === a.adresse);
      if (neuve){
        const z = document.querySelector('[data-edition="' + neuve.indice + '"]');
...
async function infos(){
  try{
    const [w, info, emp] = await Promise.all([
      appel("getwalletinfo"), appel("getinfo"), appel("getempreinteutxo")
    ]);
    document.getElementById("tuiles-infos").innerHTML =
      ...
      tuile("Schéma de signature", w.schema, "résistant à l'ordinateur quantique — voir ci-dessous") +
    ...
    document.getElementById("tuiles-chaine").innerHTML =
      ...
      tuile("Empreinte de l'état", brut(
        ...
        "une signature unique de tout l'argent qui existe : deux nœuds à la même hauteur l'affichent identique — votre preuve d'être sur la vraie chaîne, sans faire confiance à personne");
  }catch(e){
...
let joignableEnCours = false;
document.getElementById("bascule-joignable").addEventListener("click", async () => {
  if (joignableEnCours) return;
  joignableEnCours = true;
  const b = document.getElementById("bascule-joignable");
  const vers = b.getAttribute("aria-pressed") !== "true";
  try{
...
function peindreJoignable(info){
  const actif = !!info.joignable;
  const b = document.getElementById("bascule-joignable");
  b.setAttribute("aria-pressed", actif ? "true" : "false");
  b.setAttribute("aria-label", actif ? "Ne plus être joignable" : "Rendre ce nœud joignable");
  document.getElementById("joignable-titre").textContent =
    actif ? "Votre portefeuille aide le réseau" : "Votre portefeuille est un simple client";
...
async function reseau(){
  try{
    ...
    try{ peindreJoignable(await appel("getinfo")); }catch(e){ /* l'onglet reste utile sans ce detail */ }
    ...
    // La part que porte votre machine, sous son debit. Sans mesure locale — le
    // minage est eteint — on ne l'invente pas.
    const part = document.getElementById("reseau-part");
    if (mien > 0 && debit > 0){
      if (mien >= debit){
        // Le rythme mesure chez vous a l'instant depasse la moyenne du reseau
        // sur sa fenetre de blocs : soit vous portez l'essentiel des blocs
        // recents, soit vous venez d'accelerer et la moyenne n'a pas rattrape.
        // Un pourcentage compare a deux bases differentes mentirait.
        part.textContent = "à ce rythme, vous produisez l'essentiel des blocs récents";
      } else {
        const p = Math.round((mien / debit) * 100);
        part.textContent = "soit environ " + p + " % de la puissance mesurée du réseau";
      }
    } else {
...
let carnetFiltre = "";
// Les adresses derivees par le minage sont rangees par defaut : elles ont
// deja recu leur recompense et n'ont pas a etre distribuees. Le porteur peut
// les deplier, et une recherche les parcourt toujours.
let carnetTout = false;
...
// Une adresse est « a montrer » si le porteur l'a demandee ou l'a nommee.
// Les autres sont celles du minage et des restaurations.
function aMontrer(e){
  return !!(e.demandee || e.etiquette);
}
...
function rendreCarnet(){
  ...
  const minage = document.getElementById("zone-minage");
  const boutonMinage = document.getElementById("bouton-minage");
  if (!carnet.length){
  ...
  const rangees = carnet.filter(e => !aMontrer(e)).length;
  // Une recherche parcourt tout : qui tape « #12 » veut la douzieme, qu'elle
  // vienne du minage ou non. Sans recherche, on ne montre que les demandees.
  const base = (f || carnetTout) ? carnet : carnet.filter(aMontrer);
  boutonMinage.textContent = carnetTout
    ? "Ranger les adresses de minage"
    : "Afficher aussi les " + rangees + " adresse(s) de minage";
  minage.hidden = !rangees || !!f;
  const vues = base.filter(e => correspond(e, f));
  if (!vues.length && !f && !carnetTout){
    zone.innerHTML = '<div class="note"><p>Aucune adresse demandée pour ' +
      "l'instant. Le bouton « Nouvelle adresse » en crée une, que vous " +
      'pourrez nommer. Vos ' + ech(String(rangees)) + ' adresse(s) de minage ' +
      'sont rangées&nbsp;: elles ont reçu vos récompenses et comptent dans ' +
      'votre solde.</p></div>';
    compte.textContent = "";
    plus.hidden = true;
    return;
  ...
  compte.textContent = f
    ...
    : (carnetTout
        ? carnet.length + " adresse(s) au total, dont " + rangees + " de minage"
        : vues.length + " adresse(s) demandée(s)" +
          (rangees ? " · " + rangees + " de minage rangée(s)" : ""));
  plus.hidden = vues.length <= tranche.length;
}

document.getElementById("bouton-minage").addEventListener("click", () => {
  carnetTout = !carnetTout;
...
mod tests {
  ...
  return e.toString() + "." + f.toString().padStart(8, "0");
    ...
    /// La page « Recevoir » range les adresses du minage.
    ///
    /// Un mineur en possede un millier au bout de quelques jours, et n'en a
    /// distribue que deux. Les lui presenter toutes au meme rang noyait les
    /// deux qui comptent ; deux utilisateurs ont demande « pourquoi j'ai
    /// autant d'adresses ». Seules les demandees ou nommees sont montrees, une
    ...
    fn la_page_recevoir_range_les_adresses_du_minage() {
        ...
        // Une adresse neuve rejoint le carnet et ouvre son champ de nom.
        assert!(
            PAGE.contains("const neuve = carnet.find(e => e.adresse === a.adresse)"),
            "la nouvelle adresse n'est pas reprise dans le carnet"
        );
    }
    ...
    fn la_page_reseau_montre_la_machine_et_l_ensemble() {
        ...
        // Le carnet d'adresses, jadis affiche comme « machines connues du
        // reseau », a ete retire : c'etait une liste d'adresses apprises, pas
        // un decompte du reseau, et le presenter ainsi mentait par raccourci.
        // On verifie l'absence de sa tuile par son identifiant, pas par son
        // libelle — une traduction inerte du libelle peut subsister sans nuire.
        assert!(
            !PAGE.contains("reseau-carnet") && !PAGE.contains("reseau-fenetre"),
            "une tuile retiree du reseau est de nouveau presente"
        );
    ...
    /// Un nœud sans pair dit **pourquoi**, et ce qu'il faut faire.
    ///
    /// Deux etats opposes s'affichaient pareil : « je n'ai l'adresse de
    /// personne » et « j'ai frappe, personne n'a ouvert ». Le premier se
    /// repare en deux lignes, le second non — et le nouveau venu, faute de
    /// savoir lequel il a sous les yeux, conclut que le reseau est mort. La
    /// page doit donc lire `amorces_configurees` et dire, le cas echeant, quel
    /// fichier creer et ce qu'il doit contenir.
    #[test]
    fn un_noeud_sans_amorce_dit_quoi_faire() {
        let s = script();
        assert!(
            s.contains("Number(info.amorces_configurees) === 0"),
            "la page ne distingue plus « aucune adresse » de « personne n'ouvre »"
        );
        assert!(
            s.contains("Aucune adresse de départ : ce nœud ne cherche personne"),
            "le titre qui nomme la cause a disparu"
        );
        // L'instruction doit etre complete : le fichier, la ligne a y mettre,
        // et le guide. Un message qui nomme le probleme sans donner le geste
        // ne vaut guere mieux que l'ancien.
        for morceau in [
            "q21-data/amorces.txt",
            "amorce.q21.dev:21121",
            "REJOINDRE.md",
        ] {
            assert!(
                s.contains(morceau),
                "le message d'aide ne contient plus : {morceau}"
            );
        }
        // Et il ne promet pas le retour des fonds a la fin d'une etape qui ne
        // peut pas commencer.
        assert!(
            s.contains("aveugle || Number(solde.adresses_derivees) > 2"),
            "la ligne de restauration promet encore des fonds a un nœud aveugle"
        );
    }
    ...
    fn la_vue_du_reseau_ne_compte_pas_les_mineurs() {
        ...
        // La page montre ce que fait votre machine et pourquoi elle ne compte
        // pas les mineurs — jamais un nombre de mineurs invente.
        assert!(
            PAGE.contains("Votre machine"),
            "la page ne montre pas ce que fait votre machine"
        );
        assert!(
            PAGE.contains("Pourquoi on ne vous dit pas"),
            "la page ne dit pas pourquoi elle ne compte pas les mineurs"
        );
        // Et sans mesure locale, on n'invente pas de part.
        assert!(
            s.contains("mien > 0 && debit > 0"),
            "la part se calcule meme sans debit local connu"
        );
    }
